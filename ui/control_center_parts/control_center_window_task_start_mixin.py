...
class ControlCenterWindowTaskStartMixin:
    ...
    def _create_window_task_runner(
        ...
        from task_workflow.process_payload import resolve_source_workflow_task_keys

        task_manager = getattr(self.parent_window, "task_manager", None)
        if task_manager is None:
            tab_widget = getattr(self.parent_window, "workflow_tab_widget", None)
            task_manager = getattr(tab_widget, "task_manager", None)
        source_workflow_task_keys = resolve_source_workflow_task_keys(
            workflow_data,
            task_manager,
        )
        runner = WindowTaskRunner(
            ...
            source_workflow_task_keys=source_workflow_task_keys,
        )
