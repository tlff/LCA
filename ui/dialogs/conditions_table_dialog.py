...
class ConditionsTableDialog(QDialog):
    def __init__(
        ...
        self._variable_combo = NoWheelComboBox()
        self._variable_insert_button = QPushButton("插入变量")
        self._variable_names = [str(item).strip() for item in (variable_names or []) if str(item).strip()]
    ...
    def _build_ui(self) -> None:
        ...
        self._variable_combo.addItem("选择变量", "")
        for var_name in self._variable_names:
            self._variable_combo.addItem(var_name, var_name)
        self._variable_insert_button.setProperty("primary", True)
        self._variable_insert_button.clicked.connect(self._insert_selected_variable)
        preset_layout.addWidget(self._variable_combo, 1)
        preset_layout.addWidget(self._variable_insert_button, 0)
    ...
    def _insert_selected_variable(self) -> None:
        name = str(self._variable_combo.currentData() or self._variable_combo.currentText()).strip()
        if not name or name == "选择变量":
            return
